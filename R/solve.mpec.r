...
mpec.solve <- function( objFunc=function(mpec) {mpec}, cFunc, x0, mpec ,opts ) {
  ...
  mpec.color = mpec.setVarsFromVector(mpec.reset(mpec),x0)
  ...
  SPARSE_JAC =  mpec.color$CST.INEQ.FDiff@J
  eval_jac_g_structure <- ipoptr.sparse( SPARSE_JAC ) 
  ...
  # -------------------------------------------------------------------
  private = list(mpec=mpec.color, cFunc=cFunc, objFunc=objFunc,SPARSE_JAC=SPARSE_JAC,evals=list(),last_x=0)
  ...
  eval_jac_g <- function( x ,private) { 
    ...
    res = t( mpec$CST.INEQ.FDiff@J  + private$SPARSE_JAC * 0)@x 
...
#' @export
#' @example examples/example-MpecSolve.r
mpec.solve.knitro <- function( objFunc=function(mpec) {mpec}, cFunc, x0, mpec ,opts ) {
